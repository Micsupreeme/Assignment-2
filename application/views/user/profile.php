...
if($user_instance['usr_auth_level'] == 0) { //Only display an assigned lecturer for students
    ...
    echo '<label>Assigned Lecturer: ' . '<a href="' . substr($_SERVER['PHP_SELF'], 0, -1) .
        $data['user_lecturer']['usr_id'] . '">' . $data['user_lecturer']['usr_first_name'] . ' ' .
        $data['user_lecturer']['usr_last_name'] . '</a></label><br>';
}
?>
<label>Bio:</label><br>
